...
while not done:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            data = {'mall': [{'name':'KLCC','shop_list': [{'Source': START, 'Destination': END, 'Path': str(visited_nodes)}]}]}
            with open('path1.txt', 'w') as outfile:
                json.dump(data, outfile, indent=4)          
            wall = open("wall_1.txt","w")
            wall.writelines(str(maze))
            wall.close()
            done = True
            ...
            if pos[1] <= SCREEN_HEIGHT - 1 and pos[0] <= SCREEN_WIDTH - 1 :
                ...
                coor = tuple(x//HEIGHT for x in pos)
    ...
    def draw_square(row, column, grid=grid, fill=1):
        pygame.draw.rect(
            screen,
    ...
    def trace_back(goal_node, start_node, v_distances, visited_nodes, n, mazearray, diags=False, visualise=VISUALISE):
        ...
        while current_node != start_node:
            ...
            draw_square(smallest_neighbour[0], smallest_neighbour[1], grid=mazearray, fill=0)
